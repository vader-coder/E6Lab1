...
%for when each coordinate is individually altered by 0.5cm and the weights are
...
%distance magnitudes with z_prime for each rope.
    ...
    %uses position vectors with z_prime
    T2 = [T2RK(1), T2RK(2), z_prime(1)];%distance vectors w/ z correction
    T3 = [T3RK(1), T3RK(2), z_prime(2)];
    T4 = [T4RK(1), T4RK(2), z_prime(3)];
    unit = [T2/magT(1); T3/magT(2); T4/magT(3)];
    ...
    %each row contains a vector for T.
